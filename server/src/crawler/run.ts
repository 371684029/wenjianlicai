...
import { upsertProducts, deleteSamples, deleteRealData } from '../repo.js';
 ...
 * 不再用示例数据补齐；缺失覆盖由 /api/coverage 接口暴露给前端。
 */
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(`[crawler] 入库完成：真实 ${nReal} 条。缺数据请通过前端 /api/coverage 查看。`);
  } catch (err) {
