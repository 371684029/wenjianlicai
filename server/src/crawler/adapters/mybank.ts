...
const PAGE = 'https://www.mybank.cn/';
const UA =
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36';

/**
 * 网商银行 存款利率（best-effort）。
 * 网商银行为互联网银行，官网是 SPA、存款产品主要在 App 内，公开页无静态利率表；
...
export const mybankAdapter: SourceAdapter = {
  ...
  async fetch(): Promise<RawProduct[]> {
    try {
      // 仅探测可达性；SPA 无静态利率数据，暂不解析
      await axios.get<string>(PAGE, { timeout: 10000, headers: { 'User-Agent': UA } });
      console.log('[crawler] 网商银行 为 SPA，公开页无静态利率，返回 0 条');
    } catch (err) {
