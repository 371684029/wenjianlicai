...
| --- | --- | --- | --- |
| **招商** | `cmb.ts` | ✅ 真实（可靠「高」） | 官方实时金融信息站点 JSON 接口，日期较新 |
| **建设** | `ccb.ts` | ✅ 真实（可靠「高」） | 官网利率页 iframe 静态表格 |
| **平安** | `pingan.ts` | ✅ 真实（可靠「高」） | 官网利率页静态表（含执行日期） |
| **网商** | `mybank.ts` | ✅ 真实（可靠「高」） | 官网储蓄利率表；公开页**无生效日期**，互联网银行实时利率以 App 为准，公开页可能滞后 |
| **微众** | `webank.ts` | ✅ 真实（可靠「高」） | 官网公告利率表；公开页**生效日期较旧**（互联网银行实时利率以 App 为准） |

- 五家银行**定期存款利率均为真实抓取**；每条标注「数据年份」与来源，便于判断时效。
- **大额存单与理财** 目前仍为示例（各行限制）。
- 抓取主流程「**真实优先 + 按产品回退示例**」：真实产品用真实值，其余示例补齐，按 `code` 去重合并。
- **新鲜度与陈旧降权**：推荐打分按「数据日期」计算新鲜度，数据超 2 年整体降权，避免互联网银行公开页的过时高息污染推荐榜。
- 扩展/完善：在 `adapters/` 实现 `SourceAdapter` 并在 `run.ts` 的 `ADAPTERS` 注册；动态渲染站点可逆向接口或引入 `Playwright`。
