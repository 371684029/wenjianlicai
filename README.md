...
| 银行 | 定期存款利率 | 说明 |
| --- | --- | --- |
| **招商** | ✅ **真实**（可靠「高」） | 来自招商银行官方实时金融信息站点 JSON 接口（`adapters/cmb.ts`） |
| 平安 / 建设 / 网商 / 微众 | 示例（可靠「低」） | 官网为 SPA 动态渲染 / 反爬，静态抓取拿不到；大额存单与理财同理 |

- 抓取主流程为「**真实优先 + 按产品回退示例**」：已接入真实数据的产品用真实值，其余用示例补齐，保证五家银行都有内容。
- 扩展真实数据源：在 `server/src/crawler/adapters/` 新增适配器实现 `SourceAdapter` 接口，并在 `run.ts` 的 `ADAPTERS` 注册即可；产品 `code` 与示例保持一致即可自动去重合并。
- 动态渲染 / 反爬的银行（平安 / 建设 / 网商 / 微众）后续可逆向其数据接口或引入 `Playwright` 渲染后解析。
