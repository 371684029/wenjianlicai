...
- **可靠等级**：`高`＝官方权威源（银行官网 / 中国理财网）、`中`＝第三方聚合、`低`＝示例 / 参考数据。
- **数据年份**：每条数据标注采集 / 有效日期的年份，便于判断时效。
- **在售状态**：`在售`（可买，优先展示）、`售罄` / `已下架` / `待售`（当前不可买，靠后并置灰标记）。

### 各银行数据来源现状

五家银行均已写成独立适配器（`server/src/crawler/adapters/`）并注册到爬虫，逐家攻克真实数据：

| 银行 | 适配器 | 定期存款利率 | 数据来源 / 备注 |
| --- | --- | --- | --- |
| **招商** | `cmb.ts` | ✅ 真实（可靠「高」） | 官方实时金融信息站点 JSON 接口（2025-05） |
| **建设** | `ccb.ts` | ✅ 真实（可靠「高」） | `fund.ccb.com` 利率页 → 当前生效日期对应的 article 利率页（2025-05） |
| **平安** | `pingan.ts` | ✅ 真实（可靠「高」） | 官网利率页静态表（2025-05） |
| **网商** | `mybank.ts` | ✅ 真实（可靠「中」） | 官网储蓄利率表；公开页**无生效日期**，时效不可校验故标「中」 |
| **微众** | `webank.ts` | 示例（可靠「低」） | 官网公告页**长期未更新（停留 2016）**，适配器自动丢弃过时数据回退示例；实时利率在 App |

- 招商/建设/平安**定期存款为官网当前真实挂牌利率（2025-05）**；网商为官网真实但无日期（标「中」）；微众官网公开页过时已回退示例。
- **大额存单与理财产品** 仍为示例：理财数据源（中国理财网）产品接口需 token + 前端加密，纯 HTTP 抓不到，需无头浏览器（`Playwright`）执行页面 JS，后续可接入。
- **可靠等级按时效校准**（`run.ts`）：无生效日期→「中」、数据超 2 年→「低」；微众这类过时官网数据由适配器直接丢弃。
- **新鲜度与陈旧降权**（`scoring.ts`）：推荐打分按「数据日期」算新鲜度（3 年衰减），超 2 年整体降权，避免过时高息污染推荐榜。
- 抓取主流程「**真实优先 + 按产品回退示例**」：真实产品用真实值，其余示例补齐，按 `code` 去重合并。
- 扩展/完善：在 `adapters/` 实现 `SourceAdapter` 并在 `run.ts` 的 `ADAPTERS` 注册；动态渲染/加密站点可引入 `Playwright`。
