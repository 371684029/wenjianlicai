...
| 银行 | 适配器 | 定期存款利率 | 说明 |
| --- | --- | --- | --- |
| **招商** | `cmb.ts` | ✅ **真实**（可靠「高」） | 官方实时金融信息站点 JSON 接口 |
| **建设** | `ccb.ts` | ✅ **真实**（可靠「高」） | 官网利率页 iframe 静态表格解析（数据日期以官网为准） |
| 平安 | `pingan.ts` | best-effort → 示例 | 官网利率页动态渲染/易超时，静态抓不到 |
| 网商 | `mybank.ts` | best-effort → 示例 | 互联网银行，官网 SPA、产品在 App 内 |
| 微众 | `webank.ts` | best-effort → 示例 | 互联网银行，官网 SPA、产品在 App 内 |

- 抓取主流程为「**真实优先 + 按产品回退示例**」：已接入真实数据的产品用真实值，其余用示例补齐，保证五家银行都有内容。
- 大额存单与理财目前仍为示例（各行官网/App 限制）。
- 扩展/完善真实源：在 `adapters/` 实现 `SourceAdapter` 并在 `run.ts` 的 `ADAPTERS` 注册；产品 `code` 与示例一致即自动去重合并。
- 仍为 best-effort 的银行（平安/网商/微众）后续可逆向其数据接口或引入 `Playwright` 渲染后解析。
